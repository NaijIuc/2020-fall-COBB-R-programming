...
t2 <- read.table('https://raw.githubusercontent.com/jmzeng1314/my-R/master/DEG_scripts/tair/DESeq2_DEG.Day1-Day0.txt')
t3 <-  readClipboard()
...
```{r, , eval=F, echo=T}
plot(x = t2$log2FoldChange, y = t2$padj)
abline(h = 0.05, col = "red", lty = 2)

plot(x = t2$log2FoldChange, y = -log(t2$pvalue), 
     col = ifelse(t2$pvalue > 0.05, "gray", 
                  ifelse(t2$log2FoldChange < 0, "blue","red")),
     main = "My Volcano",
...
plot(sample(t2$pvalue,50))
barplot(t2$log2FoldChange[1:20])
hist(t2$pvalue)
pie(t2$baseMean[7:10], labels = rownames(t2)[7:10])
boxplot(list(t2$pvalue, t2$padj))
dotchart(t2$baseMean[1:10], labels = rownames(t2)[1:10])
```
